    ...
    return (        
        ...
        className={`${avilable_seats === 0 ? "bg-red-950 bg-opacity-40": ""} p-4 border overflow-hidden rounded-md shadow-md relative`}>
            ...
            <motion.button 
                ...
                disabled={role === "admin" || role === "instructor" || avilable_seats === 0} onClick={() => seletedClass(singleClass)} className={`${role === "admin" || role === "instructor" || avilable_seats === 0 ? "opacity-25 cursor-not-allowed" : ""} bg-main px-8 py-2 rounded text-white block my-4 absolute bottom-0 left-0 right-0 mx-4`}>Select</motion.button>
        </motion.article>
    );
};
