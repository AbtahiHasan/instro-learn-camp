...
const ClassCard = ({singleClass, seletedClass}) => {
    ...
    return (
        <article data-aos="zoom-in" className={`${avilable_seats === 0 ? "bg-red-950 bg-opacity-40": ""} p-4 border rounded-md shadow-md relative`}>
            ...
            <p className='text-center mb-16'><span className='font-bold'>price: </span> ${price}</p>
            <button disabled={role === "admin" || role === "instructor" || avilable_seats === 0} onClick={() => seletedClass(singleClass)} className={`${role === "admin" || role === "instructor" || avilable_seats === 0 ? "opacity-25 cursor-not-allowed" : ""} bg-main px-8 py-2 rounded text-white block my-4 absolute bottom-0 left-0 right-0 mx-4`}>Select</button>
        </article>
    );
};
